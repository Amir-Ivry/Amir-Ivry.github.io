...
{%- for g in groups -%}
    ...
    {%- for p in g.items -%}
      ...
      <li class="pub">
        <div class="pub-line">
          ...
          {%- assign has_links = '' -%}
          {%- if paper_url and paper_url != '' -%}{%- assign has_links = 'y' -%}{%- endif -%}
          {%- if p.code and p.code != '' -%}{%- assign has_links = 'y' -%}{%- endif -%}
          {%- if p.project and p.project != '' -%}{%- assign has_links = 'y' -%}{%- endif -%}
          {%- if p.video and p.video != '' -%}{%- assign has_links = 'y' -%}{%- endif -%}
          {%- if has_links == 'y' -%}<span class="sep">&middot;</span>{%- endif -%}
  ...
  {%- else -%}
    {%- assign other_group = g -%}
  {%- endif -%}
{%- endfor -%}

{%- if other_group and other_group.items and other_group.items.size > 0 -%}
<h3 class="pub-year">Other</h3>
<ul class="pubs">
  {%- for p in other_group.items -%}
    {%- assign paper_url = p.pdf | default: p.paper | default: p.link -%}
    <li class="pub">
      <div class="pub-line">
        <strong class="pub-title">{{ p.title }}</strong>
